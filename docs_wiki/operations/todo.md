...
1. [ ] Extract the panel-scan walk into one shared helper. The walk (skip
   ...
   (`screen.cc`).
2. [ ] Share a named constant for `"ADDON_PT_empty_state"`. The literal
   string is hardcoded independently in `space_addon.cc:178`,
...
4. [ ] Fix the stale doc-comment on `BKE_paneltypes_addon_space_types_get`.
   The comment still references the dynamic-itemf approach that was
...
5. [ ] Unify the two add-on attribution implementations. C++
   ...
   matches by module prefix. They agree today, but the comment in
...
7. [ ] Widen `addon_delegate_spacetype_find()` to search every window's
   ...
   window. Currently mitigated by hiding the Add-on Editor option in
...
9. [ ] Build the cross-window swap by following the order established in
   ...
   then `CTX_wm_region_set`. Add the window and screen calls alongside
...
11. [ ] Add a post-layout walk of the region's `uiBlock`/`uiBut` lists to
    ...
    Keep the check coarse. A finer classifier is not staticly buildable:
    `TRANSFORM_OT_translate` breaks when hosted because its
    `convertViewVec()` (`transform.cc:185-231`) needs the real region's
    `RegionView3D` state, but the installed DreamUV add-on's
...
12. [ ] Evaluate the `temp_override`-based real fix as an alternative to
    ...
    the panel's own collection already resolved. `WM_operator_call_py`
    ...
    tightly scoped swap covers the whole operator lifetime. The missing
    piece is the interception point: a button in a re-hosted panel is
    invoked by Blender's generic dispatch (`interface_handlers.cc`),
...
14. [ ] Design and implement per-panel hosting: store a list of panel
    ...
    working. A non-empty list means host exactly those panels. The
...
19. [ ] Add a runtime-only WM-level flag, not persisted DNA, set and
    ...
    call already bracketed for the area and region swap. Give hosted
    add-ons an official way to know they are running inside this editor,
...
21. [ ] Do not expose this signal through `context.area.type` or the
    ...
    editor type, not `'ADDON'`. Exposing it through the generic
...
23. [ ] Document that `context.space_data` is not the correct accessor
    ...
    the delegate's space when one is active. Document
...
25. [x] Delete `SPACE_ADDON_ID_PICK_MARKER`. **Done on 2026-09-12.**
    ...
    No versioning step was needed. `source/blender/blenloader/` holds zero
...
26. [ ] Confirm whether the global poll-failure blacklist
    ...
    standard RNA-boundary error reporting alone is sufficient. A concern
    was raised (cross-window side effects, silent permanent
    blacklisting) but not traced through to a conclusion.
...
Found by a pre-alpha audit on 2026-09-12. Each one is verified with a
repo-wide grep. **Marked, not deleted.** Delete them in one commit, after
the next rebase onto upstream `main`.

**No versioning step is needed.** The user states that no real file holds
...
A preference file written before the picker was removed can still hold
...
**Keep it.** It is a legacy read-only fallback, not dead code.
